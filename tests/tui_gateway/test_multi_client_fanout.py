...
def test_membership_preserves_terminal_delivery_and_revokes_departed_peers(monkeypatch, attachment):
    with ExitStack() as stack:
        ...
        for client, allowed in ((a, True), (b, True), (stranger, False)):
            ...
            usage = server.dispatch({"id": 2, "method": "session.usage",
                                     "params": {"session_id": "fanout-invariant"}}, client)
            if usage is None:
                usage = client.receive()
            assert ("result" in usage) == allowed, usage
            if not allowed:
                assert usage["error"]["code"] == 4403
