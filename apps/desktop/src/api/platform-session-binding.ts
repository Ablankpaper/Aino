...
export async function createPlatformDraft(
  ...
  try {
    ...
    await bindSelectedPlatformSession(
      owner,
      created.session_id,
    ...
    return {
      ...created,
      info: { ...created.info, model_status: 'ready', platform_owner: { user_id: ownerUserId, platform_origin: '' } }
    }
  } catch (error) {
...
export async function preparePlatformSessionRequest(
  ...
  await bindSelectedPlatformSession(owner, params.session_id, state.platformModel, request)
}
