...
function applyReceipt(state: ClientSessionState, update: PendingReplyBilling): ClientSessionState | undefined {
  ...
  if (current.revision === billing.revision &&
    (!update.nonAinoModelCalls || message.turnMetrics?.non_aino_model_calls)) { return state }

  const messages = state.messages.slice()
  messages[index] = { ...message, turnMetrics: {
    ...message.turnMetrics,
...
export function receiveReplyBilling(
  ...
  if (previous && (!sameTurnBillingOwner(previous.billing, billing) || previous.billing.revision > billing.revision ||
    (previous.billing.revision === billing.revision &&
      (!update.nonAinoModelCalls || previous.nonAinoModelCalls)))) { return state }

  // A title can finish between persisting a reply and publishing message.complete.

  const pending = { ...state.pendingReplyBilling, [billing.turn_id]: {
    ...update,
