...
/**
 * The peek is scoped with :has() so only the Settings surface that owns the
 * translucency row pays for an opacity transition. jsdom implements :has() in
 * matches(), so the scoping claim is checkable directly: the marked page (and
 * the legacy overlay shape) matches while an unrelated overlay does not.
 */
import { describe, expect, it } from 'vitest'

const PEEK_SELECTOR = ':is([data-settings-workspace], [data-overlay-surface]):has([data-translucency-peek-scope])'

describe('peek scoping', () => {
  it('matches only the overlay that armed the interaction', () => {
    document.body.innerHTML = `
      <div data-settings-workspace id="settings">
