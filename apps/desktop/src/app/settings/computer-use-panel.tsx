...
function PermissionRow({ granted, label, hint }: { granted: boolean | null; label: string; hint: string }) {
  const { t } = useI18n()
  const c = t.settings.computerUse
  ...
  return (
    <div className="flex flex-wrap items-center justify-between gap-2 rounded-lg bg-background/55 p-2.5">
      ...
      <Pill tone={tone(granted)}>
        ...
        {granted === true ? c.granted : granted === false ? c.notGranted : c.unknown}
      </Pill>
    </div>
  )
}
...
export function ComputerUsePanel({ onConfiguredChange }: ComputerUsePanelProps) {
  ...
  const refresh = useCallback(async () => {
    try {
      ...
      notifyError(err, c.readStatusFailed)
    } finally {
      setLoading(false)
    }
  }, [c.readStatusFailed])
  ...
  const grant = useCallback(async () => {
    ...
    try {
      ...
      if (!started.ok) {
        notifyError(new Error('spawn failed'), c.requestPermissionsFailed)
      ...
      if (activeRef.current) {
        notifyError(err, c.requestPermissionsFailed)
      }
    } finally {
      if (activeRef.current) {
        setGranting(false)
  ...
  if (!status.installed) {
    return (
      <p className="px-1 text-xs text-muted-foreground">
        {c.installDriver}
        {status.can_grant && c.grantAfterInstall}
      </p>
    )
  }
  ...
  return (
    <div className="grid gap-2">
      <div className="flex flex-wrap items-center justify-between gap-2 px-1">
        <div className="min-w-0">
          {status.can_grant ? (
            <p className="text-[0.72rem] text-muted-foreground">{c.identityNote}</p>
          ) : (
            <p className="text-[0.72rem] text-muted-foreground">
              {status.platform === 'linux' ? c.linuxNote : status.platform === 'win32' ? c.windowsNote : ''}
            </p>
          )}
      ...
      {status.can_grant ? (
        <>
          <PermissionRow granted={status.accessibility} hint={c.accessibilityHint} label={c.accessibility} />
          <PermissionRow granted={status.screen_recording} hint={c.screenRecordingHint} label={c.screenRecording} />
        </>
      ) : (
        <div className="flex flex-wrap items-center justify-between gap-2 rounded-lg bg-background/55 p-2.5">
