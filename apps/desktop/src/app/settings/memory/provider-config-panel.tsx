...
export function ProviderConfigPanel({ profile, provider }: { profile?: string; provider: string }) {
  ...
  const refresh = useCallback(async () => {
    try {
      ...
      setConfig(null)
      setLoadError(err instanceof Error ? err.message : m.loadFailed)
    }
  }, [m.loadFailed, profile, provider])
  ...
  const commitField = useCallback(
    async (field: MemoryProviderField, value: string) => {
      ...
      try {
        ...
        notifyError(err, m.saveFailed(field.label))
      }
    },
    [m, profile, provider, saved]
  )
  ...
  if (!config) {
    ...
    return <PageLoader className="min-h-24" label={m.loading} />
  }
      ...
      <div className="flex items-center gap-2 py-2">
        ...
        >
          ...
          <span className="text-[length:var(--conversation-text-font-size)] font-medium text-foreground">
            {m.settingsTitle(config.label)}
          </span>
          {secretFields.map(field => (
            <Pill key={field.key}>{field.is_set ? `${field.label} ${m.set}` : `${field.label} ${m.notSet}`}</Pill>
          ))}
        </button>
        {hasFullConfig && (
