...
function previewMessage(
  ...
  copy: ReturnType<typeof useI18n>['t']['billing']
) {
  if (phase.kind === 'previewing') {
    return copy.plan.checking
  }
  ...
  switch (preview.effect) {
    case 'blocked':
      return preview.reason ?? copy.plan.cannotMake

    case 'no_op':
      return copy.plan.alreadyOn(targetName)

    case 'scheduled':
      return copy.plan.scheduledChange(targetName, formatBillingDate(preview.effective_at), creditsDelta ?? undefined)

    default:
      return copy.plan.cannotSchedule
  }
}
...
function DowngradeConfirm({ flow, tier }: { flow: DowngradeFlow; tier: BillingPlanTierView }) {
  ...
  const message = previewMessage(phase, tier.name, t.billing)
  ...
  return (
    ...
    >
      ...
      <div className="mt-auto min-w-0 pt-1">
        {isCurrent && <Pill tone="primary">{copy.current}</Pill>}
