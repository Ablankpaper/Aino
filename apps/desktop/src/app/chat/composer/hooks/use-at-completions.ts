...
// `Translations` accepts additive brand overlays, so its nested index
// signature widens `keyof` to include number/symbol. Completion starters are
// wire-level string tokens; keep that boundary explicit.
type ReferenceDescriptionKey = Extract<keyof Translations['composer']['referenceDescriptions'], string>
