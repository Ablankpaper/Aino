...
const PURPOSES = new Set(['chat', 'title', 'compression', 'vision', 'delegation', 'other_auxiliary'])

export function parseTurnBilling(value: unknown): TurnBilling | undefined {
  if (!value || typeof value !== 'object' || Array.isArray(value)) { return undefined }
  const raw = value as Record<string, unknown>

  if (raw.source !== 'aino' || typeof raw.user_id !== 'string' || !raw.user_id || raw.user_id.length > 128 ||
    typeof raw.session_id !== 'string' || !UUID.test(raw.session_id) ||
    typeof raw.turn_id !== 'string' || !UUID.test(raw.turn_id) ||
    !Array.isArray(raw.calls) || raw.calls.length > 10000 ||
    typeof raw.calls_complete !== 'boolean' ||
    typeof raw.revision !== 'number' || !Number.isSafeInteger(raw.revision) || raw.revision < 0) { return undefined }
  ...
  for (const call of raw.calls as unknown[]) {
    ...
    if (typeof item.call_id !== 'string' || !UUID.test(item.call_id) || seen.has(item.call_id) ||
      typeof item.purpose !== 'string' || !PURPOSES.has(item.purpose)) { return undefined }
  ...
  return { source: 'aino', user_id: raw.user_id, session_id: raw.session_id, turn_id: raw.turn_id,
    status: 'pending', calls, calls_complete: raw.calls_complete, revision: raw.revision }
}
...
export function turnBillingEquivalent(a?: TurnBilling, b?: TurnBilling): boolean {
  return a === b || Boolean(a && b && sameTurnBillingOwner(a, b) && a.revision === b.revision &&
    a.calls_complete === b.calls_complete && a.calls.length === b.calls.length &&
