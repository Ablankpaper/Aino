...
describe('managed reply correlation', () => {
  it('preserves valid receipts without trusting runtime settlement or secret fields', () => {
    const raw = { source: 'aino', user_id: '17', session_id: crypto.randomUUID(), turn_id: crypto.randomUUID(),
      calls: [{ call_id: crypto.randomUUID(), purpose: 'compression' }], calls_complete: true, revision: 3,
      status: 'settled', actual_cost: '0.00', api_key: 'fixture-secret' }
    ...
    expect(turnMetricsEquivalent({ billing: parsed }, { billing: parseTurnBilling({ ...raw, revision: 4 }) })).toBe(false)
