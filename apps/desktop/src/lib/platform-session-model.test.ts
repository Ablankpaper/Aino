...
it('keeps the catalog identity through draft creation and runtime updates', () => {
  ...
  expect(platformCreateOverrides('aino', 'catalog-a', 'user-a', account, catalog))
    .toEqual({ model_source: 'aino', model_id: 'catalog-a' })
  const patch = platformModelStatePatch({ model_source: 'aino', model_id: 'catalog-a', model: 'wire-model',
...
it('never silently switches a saved platform choice to a different account', () => {
  expect(() => platformCreateOverrides('aino', 'catalog-a', 'user-a', platformSnapshot('user-b'), [platformModel()])).toThrow()
  expect(platformCreateOverrides('custom:local', 'byok-model', '', null, []))
    .toEqual({ model: 'byok-model', provider: 'custom:local' })
})
