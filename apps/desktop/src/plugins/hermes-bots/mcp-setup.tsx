...
export function McpSetupButton({ profile, entry, onDone, ensureProfile }: McpSetupButtonProps) {
  ...
  const beginKeys = async () => {
    ...
    if (entry.fromCatalog && !entry.installed) {
      ...
      if (!add.ok) {
        setPhase('error')
        setMessage(add.error || b.mcp.couldNotAddServer)
  ...
  const submitKeys = async () => {
    ...
    if (!target) {
      setPhase('error')
      setMessage(b.mcp.noTargetProfile)
    ...
    for (const k of requires) {
      ...
      if (!r.ok) {
        setPhase('error')
        setMessage(r.error || b.mcp.failedToSet(k))
    ...
    if (t.ok && t.result && (t.result.ok || (t.result.result && t.result.result.ok))) {
      ...
      host.notify({
        kind: 'success',
        message: b.mcp.configured(entry.name)
      })
      onDone && onDone()
    } else {
      setPhase('error')
      setMessage((t.result && (t.result.error || (t.result.result && t.result.result.error))) || b.mcp.serverTestFailed)
    }
  }
  ...
  const beginOAuth = async () => {
    ...
    if (!start.ok || !authUrl || !sessionId) {
      ...
      setPhase('error')
      setMessage(start.error || b.mcp.oauthStartFailed)
    ...
    if (listener) {
      ...
      void (async () => {
        ...
        if (!relay.ok || (rp && rp.ok === false)) {
          setPhase('error')
          setMessage((rp && rp.error_message) || relay.error || b.mcp.oauthCallbackFailed)
        }
      })()
    }
    ...
    setPhase('oauth')
    setMessage(b.mcp.completeSignIn)
    pollRef.current = setInterval(async () => {
      ...
      if (status === 'approved') {
        ...
        host.notify({
          kind: 'success',
          message: b.mcp.authenticated(entry.name)
        })
        onDone && onDone()
        ...
        pollRef.current = null
        setPhase('error')
        setMessage((pd && pd.error_message) || b.mcp.oauthFailed)
      }
    }, 2000)
  }
  ...
  if (supported === false) {
    return (
      <span className="ml-1.5 text-[0.65rem] text-(--ui-text-quaternary)">
        {b.mcp.setupNeeded(requires.join(', '))}
      </span>
    )
  }

  if (phase === 'done') {
    return <span className="ml-1.5 text-[0.65rem] text-(--ui-success)">{b.mcp.setupDone}</span>
  }
  ...
  if (phase === 'oauth') {
    return <span className="ml-1.5 text-[0.65rem] text-(--ui-text-quaternary)">{message || b.mcp.authorizing}</span>
  }
  ...
  return (
    <Button
      ...
      variant="link"
    >
      {isOAuth ? b.mcp.signIn : b.mcp.setUp}
    </Button>
  )
}
