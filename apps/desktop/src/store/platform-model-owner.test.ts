...
it('discards an old authority after the account mode changes, even with the same id and revision', async () => {
  ...
  const owner = createPlatformModelOwner(account, () => new Promise(resolve => { complete = resolve }))
...
it('publishes only the matching authority and invalidates it on account revision changes', async () => {
  ...
  let user = 'other-user'
  const owner = createPlatformModelOwner(account, async () => ({ user_id: user, platform_origin: 'http://127.0.0.1:7001' }))
  await owner.load()
  expect(owner.state.get()).toEqual({ phase: 'error', owner: null })
  user = 'user-a'
  await owner.load()
  expect(owner.state.get()).toEqual({ phase: 'ready', owner: { user_id: user, platform_origin: 'http://127.0.0.1:7001' } })
