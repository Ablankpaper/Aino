...
def open_db(
    ...
    *,
    ...
    try:
        ...
        initializing = True
    # Coordinate first creation with SessionDB's invalid-header quarantine.
    startup = quarantine_cross_process_lock(path) if initializing else contextlib.nullcontext(True)
    with startup as acquired:
        if not acquired:
            raise sqlite3.OperationalError(f"{db_label} startup quarantine lock timed out")
        ...
        conn = connect_tracked(path, connect_fn=sqlite3.connect, timeout=busy_timeout_ms / 1000,
