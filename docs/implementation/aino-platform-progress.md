...
剩余项：Windows/Linux 安全存储及清理、拒绝/锁定 Keychain、SSH/TLS、长时与其余原生能力矩阵、真实短信/captcha、真实模型扣费和支付/退款、签名/公证及上线。既有 `session.compress` 在 `would_grow` 时仍返回 compressed 的状态问题另列未关闭，不混入本次修复。E37 已证明其历史未签名 macOS 包默认 sandbox 可运行，不能替代本配对版本的正式发行验收。本轮仅本地提交，没有推送、合并 main、部署或真实付费操作。
...
| D 完整验收与部署准备 | E38 压缩 6 项、E39 当前配对原生业务/构建、E40 启动验收与 production bundle 恢复已记录；E26–E37 保持历史归属 | 正常签名发行、其余原生／多平台／远程与真实服务未完成 |
