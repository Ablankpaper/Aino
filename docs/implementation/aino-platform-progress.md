...
最新本地检查点：Aino `7a215f2b1159efab4a10e30eb960944865447783`，Aino-API `97e4a4812e1d2144641287774997ebd4b0235504`，均在 `codex/aino-platform-identity-models-billing`。Aino 的 B6 切换复核修正与原生 D1 继续进行，尚非最终交付 SHA；C4 来源标记已复核通过。未推送、未合并 main、未部署或真实付费测试。逐项条件见 [验收矩阵](aino-platform-acceptance-matrix.md)。

| 范围 | 当前事实 | 剩余工作 |
| --- | --- | --- |
| A、B1–B5 | 已有本地实现/分层集成证据保留 | 真实短信/付费模型、多平台验收不冒充通过 |
| B6 首次发送、作用域与账户草稿 | `52ec689c98`、`fde0064f71` 首发/恢复；`6acd2046ca` 账户+连接+profile 默认值、设置同步草稿、Activity 保留草稿/附件、旧账户会话只读；独立复核通过，454 项覆盖回归通过 | 正在补模型切换事务；后续能力/错误恢复、精确开发 origin 和完整原生 UI 验收未完成 |
...
| D Python 夹具 | `2716c31ea9` 修复五份隔离/时序测试，185 通过、3 原有跳过；独立复核通过 | TTFB 有界诊断已停止：人工 60/75 秒整文件预算耗尽，非固定卡死；最终按仓库原有 300 秒默认门禁复验，尚未通过 |
| D1 API 真实内部闭环与升级 | `97e4a4812e`：真实 JWT/托管 Key/代理/用量/账本/验签回调串联，32 顶层/38 含子项通过；真实 pre-239 schema 升级保留旧账户/余额/订单/订阅/普通 Key；独立复核通过 | 实际 Python Agent/原生 Electron 与这些服务串联正在实施；旧服务二进制回退未验证 |
| D 完整验收与部署准备 | 用户/部署/回退操作单已建立；API 新生成物无漂移，服务构建和默认测试 50 包通过；网站 types/lint/build 通过 | 手机号最小灰度限制、最终矩阵、原生闭环和多平台/正式服务验证尚未完成 |
