...
| B6 首次发送、作用域与账户草稿 | 历史首发、工具、工作区、双窗口和隐藏恢复保留；本轮 E28 并发刷新／退出与 E29 断网／撤销恢复原生通过；菜单及离线引导缺陷修复 | E31–E33 新增错误恢复／账户与站点隔离通过；晚到租约、共享后台／远程及完整能力矩阵仍未覆盖 |
...
| D 完整验收与部署准备 | E31–E34 本轮三条成功原生切片、按钮修复、完整计划回归及395文件开发构建已记录；E26–E30保持历史归属 | 正常签名发行、其余原生／多平台／远程与真实服务未完成 |
