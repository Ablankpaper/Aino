...
最新已复核本地代码检查点：Aino `d0e248b7587c035d6546677ff317ea96941d6b4a`，Aino-API `6f73d6a5833266f4eca8df986dbda73e6c2f9aaf`，均在 `codex/aino-platform-identity-models-billing`。B6 仍有能力/恢复 UX 与精确平台 origin 待完成，D1 原生完整链路及最终交付继续进行；模型切换、C4 来源标记和短信灰度均已复核通过。尚非最终交付 SHA，未推送、未合并 main、未部署或真实付费测试。逐项条件见 [验收矩阵](aino-platform-acceptance-matrix.md)。

| 范围 | 当前事实 | 剩余工作 |
| --- | --- | --- |
| A、B1–B5 | 已有本地实现/分层集成证据保留 | 真实短信/付费模型、多平台验收不冒充通过 |
| B6 首次发送、作用域与账户草稿 | `52ec689c98`、`fde0064f71` 首发/恢复；`6acd2046ca` 账户+连接+profile 默认、草稿/附件/旧账户只读；`82e0ba3cc5`/`c3daa97bef` 模型切换修复；均独立复核通过 | 能力/错误恢复、精确开发 origin 和完整原生 UI 验收未完成 |
...
| D Python 夹具 | `2716c31ea9` 原五份修复已复核；最新四目录回归 18,572 通过，TTFB 原有默认时限内通过 | 3 项失败＋1 文件进程崩溃正定向处理，不把整套标全绿 |
