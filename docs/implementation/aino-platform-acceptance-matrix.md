...
| MODEL-11 | 部分，撤销恢复原生通过 | E29 原请求撤销后稳定错误及账户恢复入口、零新增消费、无自动重放、重新登录后主动发送恢复；其余余额不足／429／服务异常原生分支仍以 E3/E4 分层证据为界 |
| MODEL-12 | 部分 | E3 缓存／工具／历史；E26 修复 SQLite 活库竞态，新较广回归未见 SIGBUS；E27 UI 7,982/1 后定向关闭，E29 恢复工具回合通过。其余原功能原生 QA／整套单命令全绿未外推 |
