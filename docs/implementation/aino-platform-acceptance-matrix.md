...
| DESK-03 | 部分，双窗口一致通过 | E1/E2：刷新单飞、广播、退出和旧结果拒绝的分层验证；E21 双窗口实际账户/钱包一致；原生并发刷新/退出竞态未完成 |
| DESK-04 | 部分 | 实际 macOS 加密、重启恢复、退出清理已有 E1；Windows/Linux、拒绝/锁定 Keychain 未验证 |
| DESK-05 | 部分，隐藏恢复通过 | E21 实际隐藏/显示保持账户及钱包；断网不误清账户与有限恢复有 E1/E5 分层证据，完整原生断网恢复仍未验收 |
...
| MODEL-12 | 部分 | E3 续租保持缓存与工具/历史不变；E23 在 `8e8699e4b1` 完整 UI 7,979 项通过；清理修复 E24 定向通过；Python 整套 SIGBUS、其余原生原功能 QA 仍单列未完成 |
...
2026-09-17 追加定向诊断：Python 原生 fault 位于 SQLite WAL 调用栈，系统报告映射 page-in past EOF；实际映射文件与导致越过 EOF 的操作/actor 未确定，不能单凭该错误预设 truncate。单次带时序实验排除了逐 fixture 删除目录的解释，没有复现 SIGBUS。打包 A/B 仅证明外层测试沙箱下的差异，不能归结为已证明的普通安装源码缺陷。两项门禁保持开放，详见[本地发行门禁记录](aino-platform-local-release-gates.md)。
