...
原生压缩账本结算、`would_grow` 状态报告、远程/跨平台、长时能力及真实服务/签名发行仍未关闭；本表不是上线通过单。
...
| E38 | [压缩恢复回执](aino-platform-compression-20260918.json)，`c0620226c0` | 实际 gateway/Agent/SDK，Chat Completions、Anthropic Messages、Responses × 默认/命名工作区，共 6 项/29.3 秒；身份、摘要回放、重新绑定、计费关联与分支隔离。不是原生/API 账本压缩结算；`would_grow` 状态另列未关闭 |
...
| MODEL-07 | 部分 | E18/E39 重启后 fresh 登录、新 Python 进程历史重绑定及工具回合通过；E38 实际三协议/两工作区压缩→关闭→非懒恢复→重新绑定→发送及分支 6 项通过。原生压缩/API 账本结算、自动记住登录与 `would_grow` 状态仍不在通过范围 |
...
| MODEL-10 | 部分，指定并发隔离原生通过 | E32/E33 独立应用并发及旧历史恢复通过；E35 共享后台托管授权、E36 迟到凭据换账户通过；远程／其他拓扑仍未验收 |
...
| MONEY-03 | 隔离原生及分层通过 | E35 费用最终 manifest/账本到齐后收敛合同，E36/E37 原生 Charged 标签与等待期零增费；E3/E5 多 call/晚到辅助/分页/pending；E18 为 5 call/3 turn 共 0.0005 USD、BYOK 不增加消费；E21 独立工作区 2 usage/0.0002 USD 与账本一致，非线上账单 |
