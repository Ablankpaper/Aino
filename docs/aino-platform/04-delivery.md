...
本次没有全量重跑、真实供应商、推送、合并或部署。E37 的历史未签名 macOS 默认 sandbox 成功保留原源码归属；当前结果不是新签名发行包或全新安装验收。`would_grow` 状态报告、原生压缩结算、完整原生/远程/多平台、真实短信/captcha/模型扣费/支付和正式签名上线仍开放，不能全选下列 D 门禁。
